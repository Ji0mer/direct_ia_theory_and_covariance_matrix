...
def execute(block, config):
    ...
    for c,s1,s2 in zip(corrs,sample_a,sample_b):
        ...
        if (c=='wgg'):
            za, W = get_redshift_kernel(block, 0, 0, z1, block['distances','z'], block['distances','d_m'], s1, s2)
            #import pdb ; pdb.set_trace()
            #Wofz = interp1d(W,za)
...
def get_redshift_kernel(block, i, j, z0, z_x, x, sample_a, sample_b):
    
    interp_x = spi.interp1d(z_x,x,fill_value='extrapolate')
    x = interp_x(z0)
