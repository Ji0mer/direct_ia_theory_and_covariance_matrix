...
class NlgalNlgalSpectrum(LingalLingalSpectrum):
    ...
    def set_power(self, block, bin1, bin2):
        ...
        order = ["b1E", "b2E", "bsE", "b3nlE", "bkE"]
        vec1 = np.array([self.bias_values_a[bin1][k] for k in order], dtype=float)
        vec2 = np.array([self.bias_values_b[bin2][k] for k in order], dtype=float)
        block["galaxy_power","bias_values_a[bin1]"] = vec1
        block["galaxy_power","bias_values_b[bin2]"] = vec2
