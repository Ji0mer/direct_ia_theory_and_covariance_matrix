...
class NlgalNlgalSpectrum(LingalLingalSpectrum):
    ...
    def load_bias(self, block, sample, pt_type):
        ...
        for bin_num in range(1, nbin+1):
            bias_values[bin_num] = get_bias_params_bin(block, 
                bin_num, pt_type, "bias_%s"%sample)
    ...
    def set_power(self, block, bin1, bin2):
        ...
        pks_path = block["galaxy_power","pks_folder"] #"/home/yang.zep/research/direct_ia_theory/test_output/Pks/"
        block["galaxy_power","bias_values_a[bin1]"] = [self.bias_values_a[bin1]['b1E'],
        self.bias_values_a[bin1]['b2E'],self.bias_values_a[bin1]['bsE'],self.bias_values_a[bin1]['b3nlE'],
        self.bias_values_a[bin1]['bkE']]
        block["galaxy_power","bias_values_b[bin2]"] = [self.bias_values_b[bin2]['b1E'],
        self.bias_values_b[bin2]['b2E'],self.bias_values_b[bin2]['bsE'],self.bias_values_b[bin2]['b3nlE'],
        self.bias_values_b[bin2]['bkE']]
        block["galaxy_power","Pnl"] = self.Pk_basis_funcs["Pnl"]
        block["galaxy_power","Pd1d2"] = self.Pk_basis_funcs['Pd1d2']
        block["galaxy_power","Pd2d2"] = self.Pk_basis_funcs['Pd2d2']
        block["galaxy_power","Pd1s2"] = self.Pk_basis_funcs['Pd1s2']
        block["galaxy_power","Pd2s2"] = self.Pk_basis_funcs['Pd2s2']
        block["galaxy_power","Ps2s2"] = self.Pk_basis_funcs['Ps2s2']
        block["galaxy_power","sig3nl"] = self.Pk_basis_funcs['sig3nl']
        block["galaxy_power","k2P"] = self.Pk_basis_funcs['k2P']
        if folder_has_files(pks_path):
            ...
            """
            dp1 = self.Pk_basis_funcs['Pnl'] - np.load(pks_path+"Pk1_Pd1d1.npz")['arr_0']
            dp2 = self.Pk_basis_funcs['Pd1d2'] - np.load(pks_path+"Pk2_Pd1d2.npz")['arr_0']
            dp3 = self.Pk_basis_funcs['Pd2d2'] - np.load(pks_path+"Pk3_Pd2d2.npz")['arr_0']
            dp4 = self.Pk_basis_funcs['Pd1s2'] - np.load(pks_path+"Pk4_Pd1s2.npz")['arr_0']
            dp5 = self.Pk_basis_funcs['Pd2s2'] - np.load(pks_path+"Pk5_Pd2s2.npz")['arr_0']
            dp6 = self.Pk_basis_funcs['Ps2s2'] - np.load(pks_path+"Pk6_Ps2s2.npz")['arr_0']
            dp7 = self.Pk_basis_funcs['sig3nl'] - np.load(pks_path+"Pk7_sig3nl.npz")['arr_0']
            dp8 = self.Pk_basis_funcs['k2P'] - np.load(pks_path+"Pk8_k2P.npz")['arr_0']
            
            with open(pks_path+"diffpks.txt",'a') as fdiffpks:
                np.savetxt( fdiffpks,[np.max(abs(dp1)),np.max(abs(dp2)),np.max(abs(dp3)),
                np.max(abs(dp4)),np.max(abs(dp5)),np.max(abs(dp6)),np.max(abs(dp7)),np.max(abs(dp8))] )
            with open(pks_path+"coe.txt",'a') as fcoe:
                fcoe.write( str(self.bias_values_a[bin1]) )
                fcoe.write( str(self.bias_values_b[bin2]) )
                fcoe.write("\n")
            with open(pks_path+"coe.txt",'a') as fcoe:
                fcoe.write( str(self.bias_values_a[bin1]) )
                fcoe.write( str(block["galaxy_power","bv1"]) )
                fcoe.write("\n")
                fcoe.write( str(self.bias_values_b[bin2]) )
                fcoe.write( str(block["galaxy_power","bv2"]) )
                fcoe.write("\n")
                fcoe.write("\n")
            """
            ...
            P_gg, P_gg_terms = get_PXX(self.bias_values_a[bin1], 
                self.bias_values_b[bin2], self.Pk_basis_funcs, 
                self.pt_type)
            np.savez( pks_path+"Pk1_Pd1d1.npz",self.Pk_basis_funcs["Pnl"] )
            np.savez( pks_path+"Pk2_Pd1d2.npz",self.Pk_basis_funcs['Pd1d2'] )
            np.savez( pks_path+"Pk3_Pd2d2.npz",self.Pk_basis_funcs['Pd2d2'] )
            np.savez( pks_path+"Pk4_Pd1s2.npz",self.Pk_basis_funcs['Pd1s2'] )
            np.savez( pks_path+"Pk5_Pd2s2.npz",self.Pk_basis_funcs['Pd2s2'] )
            np.savez( pks_path+"Pk6_Ps2s2.npz",self.Pk_basis_funcs['Ps2s2'] )
            np.savez( pks_path+"Pk7_sig3nl.npz",self.Pk_basis_funcs['sig3nl'] )
            np.savez( pks_path+"Pk8_k2P.npz",self.Pk_basis_funcs['k2P'] )

        #with open(pks_path+"bv1.json","w") as fbv1:
        #    json.dump(self.bias_values_a[bin1],fbv1)
        #with open(pks_path+"bv2.json","w") as fbv2:
        #    json.dump(self.bias_values_b[bin2],fbv2)
        ...
        block["galaxy_power", "k_h"] = self.k_nl_bias
        block["galaxy_power", "z"] = z0
        block["galaxy_power", "p_k"] = P_gg_div_bias
        block["galaxy_power","_cosmosis_order_p_k"] = block["matter_power_nl","_cosmosis_order_p_k"]
      ...
      #  import pdb ; pdb.set_trace()
        ...
        np.savetxt('/physics2/ssamurof/des/code/direct_ia/data/theory/pk/nlgal_power/k_h.txt', self.k_nl_bias)
      #  np.savetxt('/physics2/ssamurof/des/code/direct_ia/data/theory/pk/nlgal_power/z.txt', block['matter_power_nl','z'])
      #  np.savetxt('/physics2/ssamurof/des/code/direct_ia/data/theory/pk/nlgal_power/p_k.txt', P_gg)
...
class NlgalShearSpectrum(LingalShearSpectrum):
    """
    ...
    def set_power(self, block, bin1, bin2):
        ...
        #We inherit the compute method (and thus 
        #compute_limber and compute_exact methods)
        #from LingalShearSpectrum, which expects the 
        #non-linear P(k) in self.pk_chi_logk_spline, and 
        #then multiplies by the galaxy bias after the fact.
        #So we set self.pk_chi_logk_spline to 
        #P_gm / lin_bias_1
        ...
        block["matter_galaxy_power", "k_h"] = self.k_nl_bias
        block["matter_galaxy_power", "z"] = z0
        block["matter_galaxy_power", "p_k"] = P_gm
        block["matter_galaxy_power","_cosmosis_order_p_k"] = block["matter_power_nl","_cosmosis_order_p_k"]
...
class SpectrumCalculator(object):
    ...
    def __init__(self, options):
        # General options
        self.pks_folder = options.get_string(option_section,"pks_folder")
        os.makedirs(self.pks_folder, exist_ok=True)
    ...
    def execute(self, block):
        ...
        block["galaxy_power","pks_folder"]=self.pks_folder

        for x in self.req_kernel_keys:
            if (x[0]=='N'):
                name = str(x[1])
                block['bias_parameters',str(self.lin_bias_prefix)+'_%s'%name] = block['bias_%s'%name,'b1E_bin1']
